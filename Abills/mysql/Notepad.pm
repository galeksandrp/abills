...
sub notepad_add_note {
  ...
  $self->query2("INSERT INTO notepad (  notified, 
                        create_date, 
                        status, 
                        subject, 
                        text,
...
sub notepad_del_note {
  ...
  if ($#WHERE_RULES > -1) {
    ...
    $self->query2("DELETE from notepad WHERE $WHERE;", 'do');
  }
  return $self->{result};
}
...
sub notepad_new {
  ...
  $self->query2("SELECT sum(if(DATE_FORMAT(notified, '%Y-%m-%d') = curdate(), 1, 0))+0, sum(if(status = 0, 1, 0))+0
    FROM (notepad n)
   $WHERE;",
   undef,
   $attr
