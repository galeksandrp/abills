...
sub summary_add {
  ...
  $self->query(
    $db, "INSERT INTO extfin_reports (period, bill_id, sum, date, aid)
...
sub balances_add {
  ...
  $self->query(
    $db, "INSERT INTO extfin_balance_reports (period, bill_id, sum, date, aid)
...
sub extfin_report_balances {
  ...
  ($self->{TOTAL_DEBIT}, $self->{TOTAL_CREDIT}, $self->{TOTAL_SALDO},) = @{ $self->{list}->[0] };
...
sub paid_periodic_add {
  ...
  foreach my $id (@ids_arr) {
    $self->query(
      $db, "INSERT INTO extfin_paids_periodic 
...
sub paid_periodic_del {
  ...
  $self->query(
    $db, "DELETE FROM extfin_paids_periodic 
...
sub paid_periodic_list {
  ...
  $self->query(
    $db, "SELECT pt.id, pt.name, if(pp.id IS NULL, 0, pp.sum), 
...
sub paid_info {
  ...
  $self->query(
    $db, "SELECT date, sum, comments, uid, aid, 
  ...
  ($self->{DATE}, $self->{SUM}, $self->{DESCRIBE}, $self->{UID}, $self->{AID}, $self->{STATUS}, $self->{STATUS_DATE}, $self->{TYPE}, $self->{EXT_ID}, $self->{MACCOUNT_ID}) = @{ $self->{list}->[0] };
...
sub paids_list {
  ...
  $self->query(
    $db, "SELECT p.id, p.date, u.id, p.sum, pt.name, p.comments, p.maccount_id, a.id, p.status, 
...
sub paid_type_add {
  ...
  $self->query(
    $db, "INSERT INTO extfin_paids_types 
   (name, periodic)
...
sub paid_type_del {
  ...
  $self->query(
    $db, "DELETE FROM extfin_paids_types 
    WHERE id='$attr->{ID}';", 'do'
  );
...
sub paid_type_info {
  ...
  ($self->{ID}, $self->{NAME}, $self->{PERIODIC}) = @{ $self->{list}->[0] };
...
sub extfin_debetors {
  ...
  $self->query(
    $db, "SELECT \@uid:=u.uid, u.id, pi.contract_id,
   pi.fio,
...
sub report_payments_fees {
  ...
  if ($self->{TOTAL} > 0 || $PG > 0) {
    ...
    $FEES_WHERE =~ s/p\./f\./g;
    $self->query(
      $db, "SELECT count(DISTINCT u.uid), 
    ...
    ($self->{USERS_TOTAL}, $self->{PAYMENTS_TOTAL}, $self->{FEES_SUM}) = @{ $self->{list}->[0] };
  }
...
sub report_users_balance {
  ...
  $GROUP = 'u.uid';
  $self->query(
    $db, "SELECT u.id, pi.fio, \@payments := (select sum(p.sum) FROM payments p WHERE u.uid=p.uid $PAYMENTS_WHERE), 
       \@fees := sum(f.sum), 
  ...
  if ($self->{TOTAL} > 0 || $PG > 0) {
    $self->query(
      $db, "SELECT count(DISTINCT u.uid), sum(if(company.id IS NULL, b.deposit, cb.deposit)), 
