...
sub form_select {
  ...
  $self->{SELECT} = "<select name='$name' class='styled_selectbox' $ex_params ID='$name'>\n";
...
sub menu () {
  ...
  my $menu_text = "
<div class='inner-left_col' id='kabinet-left_col'><!--left menu-->
<ul class='inner_menu box_shadow border_rad' id='kabinet-menu'>\n";
...
sub table {
  ...
  $self->{table} = "<div class='kabinet-styled_table-wrap box_shadow border_rad'>
<table class='kabinet-styled_table' id='kabinet-filter_table' summary=''>\n";
  ...
  if ($#header_obj > -1) {
    ...
    #   $self->{table} .= $obj. '&nbsp;&nbsp;&nbsp;';
    # }
    $self->{table} .= "$attr->{header}</li></ul></div></td><td>&nbsp;</td></tr>\n";
  }

  $self->{table} .= "<tr><td class=cel_border colspan='3'><table>\n";
...
sub table_title {
  ...
  my $i = 1;
  foreach my $line (@$caption) {
    $self->{table_title} .= "<th class='table_title'>";
    if ($line ne '-') {
      if ($sort != $i) {
        $img = 'filter_i';
      }
      ...
      #$self->{table_title} .= $self->button("1", "");
      $self->{table_title} .= "<a href='index.cgi?$op$qs&pg=$pg&sort=$i&desc=$desc' class='$img noprint'>$line</a>";
    }
    else {
      $self->{table_title} .= '';
    }
