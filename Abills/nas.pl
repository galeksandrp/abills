...
sub stats_dslmax {
  ...
  my $output =
  `id=\`$SNMPWALK -c "$SNMP_COM" -v1 $NAS->{NAS_IP} RFC1213-MIB::ipRouteIfIndex.$IP | awk '{ print \$4 }'\`; a=\`$SNMPWALK -c "$SNMP_COM" -v1 $NAS IF-MIB::ifInOctets.\$id | awk '{print \$4}'\`; b=\`$SNMPWALK -c "$SNMP_COM" -v1 $NAS IF-MIB::ifOutOctets.\$id | awk '{print \$4}'\`; echo \$a \$b`;
...
sub hangup_openvpn {
  ...
  my @commands = (">INFO:OpenVPN Management Interface Version 1 -- type 'help' for more info\tkill $USER", "SUCCESS: common name '$USER' found, 1 client(s) killed\texit");
...
sub hangup_mpd4 {
  ...
  my @commands = ("\t", "Username: \t$NAS->{NAS_MNG_USER}", "Password: \t$NAS->{NAS_MNG_PASSWORD}", "\\[\\] \tbundle $ctl_port", "\] \tclose", "\] \texit");
