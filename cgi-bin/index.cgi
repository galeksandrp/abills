...
if ($uid > 0) {
  ...
  my @m = ("10:0:$_USER_INFO:form_info:::",);
...
sub form_info {
  ...
  if ($conf{user_chg_pi}) {
    $html->form_main(
      {
        CONTENT => $html->form_input('chg', "$_CHANGE", { TYPE => 'SUBMIT', OUTPUT2RETURN => 1 }),
        HIDDEN  => {
          sid   => $sid,
          index => "$index"
        }
            ...
            if ($Payments->{errno}) {
              ...
              if ($transfer_price > 0) {
                ...
                $Fees->take(
                  ...
                  $transfer_price,
                  {
                    DESCRIBE => "$_USER: $user2->{UID} $_COMMISSION",
                    METHOD   => 4,
                  }
                );
                if (!$fees->{errno}) {
